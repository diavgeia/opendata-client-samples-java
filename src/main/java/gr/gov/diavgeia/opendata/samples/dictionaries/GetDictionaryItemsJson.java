...
import gr.gov.diavgeia.opendata.json.DictionaryItem;
import gr.gov.diavgeia.opendata.json.DictionaryItems;
import gr.gov.diavgeia.opendata.util.StringUtil;
import gr.gov.diavgeia.opendata.util.JsonUtil;

public class GetDictionaryItemsJson {
    public static void main(String[] args) throws Exception {
        ...
        if (response.getStatusCode() == HttpURLConnection.HTTP_OK) {
            ...
            DictionaryItems items = JsonUtil.fromString(body, DictionaryItems.class);
            System.out.println("Dictionary: " + items.getName());
            for (DictionaryItem item : items.getItems()) {
